...
fn allow_runtime_helper_borrow(func: &Expr, index: usize) -> bool {
    match runtime_helper_name(func).as_deref() {
        ...
        Some("list_head_or_default_u32") => index == 0,
        Some("list_second_or_default_u32") => index == 0,
...
mod tests {
    ...
    #[test]
    fn accepts_defaulting_list_runtime_helper_borrows() {
        let accepted = validate_generated_ownership(
            "pub fn head(xs: Vec<u32>) -> u32 { crate::runtime::list_head_or_default_u32(&(xs), 0) }",
        )
        .unwrap();
        assert_eq!(accepted.approved_reference_exprs, 1);
        assert!(accepted.violations.is_empty());
        ...
        let accepted = validate_generated_ownership(
            "pub fn second(xs: Vec<u32>) -> u32 { crate::runtime::list_second_or_default_u32(&(xs), 0) }",
        )
        .unwrap();
        assert_eq!(accepted.approved_reference_exprs, 1);
        assert!(accepted.violations.is_empty());
