...
def facts : List ProofFact := [
  ...
  { name := "surface_expr_constructor_coverage", statement := LeanRustCore.SurfaceCoverage.surfaceCoverageSummary },
  ...
  { name := "coverage_dashboard_evidence_derived", statement := "coverage dashboard entries are generated with explicit implementation, test, documentation, generated-example, and diagnostic evidence rather than declaration-only feature rows" },
  ...
  { name := "target_interpreter_all_emitted_functions", statement := "rust/tests/target_interpreter.rs iterates every emitted target-validation function, generates snapshot-typed sample inputs, interprets the fingerprint, and compares it with the compiled generated Rust call" },
  ...
  { name := "extraction_metadata_preserved", statement := "preservation theorem proves supported ExtractIR metadata keeps source name, Rust name, and feature tags for a representative extracted declaration" },
  { name := "dependent_erasure_runtime_carriers_preserved", statement := "preservation theorem proves Subtype, Fin, Vector, and equality-cast fixtures keep the admitted runtime carriers and checks" },
  { name := "checked_surface_typing_preserved", statement := "preservation theorem proves the checked Surface type gate keeps the declared return type for a representative extracted function" },
  { name := "checked_surface_evaluation_preserved", statement := "preservation theorem proves the checked Surface evaluator agrees with a representative extracted function result" },
  { name := "target_lowering_snapshot_preserved", statement := "preservation theorem proves target-validation v2 keeps representative lowered enum/function fingerprints and module counts" },
  { name := "safe_subset_emission_preserved", statement := "preservation theorem proves the direct emitted Rust lane excludes unsafe/raw-ABI/mutable-reference/explicit-lifetime forms" },
  { name := "emitted_subset_target_semantics_preserved", statement := "preservation theorem proves representative struct, closure, dictionary, effect, and recursive target terms evaluate as documented" },
...
def reportJson : String :=
  ...
  "    \"surface_expr_constructor_coverage\": true,\n" ++
